...
BEGIN
    ...
    THEN
        RAISE EXCEPTION 'Los parÃ¡metros no pueden ser NULL.';
    END IF;
    ...
    THEN
        RAISE EXCEPTION 'El turno no estÃ¡ disponible para asignar.';
    END IF;
...
BEGIN
    ...
    IF v_id_turno IS NULL THEN
        ...
        RAISE NOTICE 'No se encontrÃ³ un turno asignado para cancelar.';
        RETURN;
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'El contenido de la historia clÃ­nica no puede estar vacÃ­o';
    END IF;
...
BEGIN
    ...
    IF p_id_rol = 2 THEN
        ...
        IF p_id_especialidad IS NULL THEN
            RAISE EXCEPTION
                'id_especialidad no puede ser NULL para mÃ©dicos.';
        END IF;


        IF p_id_obra_social IS NULL THEN
            RAISE EXCEPTION
                'id_obra_social no puede ser NULL para mÃ©dicos.';
        END IF;
...
BEGIN
    ...
    IF v_id_turno_viejo IS NULL THEN
        ...
        RAISE EXCEPTION
            'No se encontrÃ³ un turno asignado para el paciente.';
    ...
    THEN
        RAISE EXCEPTION
            'El nuevo turno no estÃ¡ disponible.';
    END IF;
